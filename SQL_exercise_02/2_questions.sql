...
SELECT e.*, d.name FROM employees e JOIN departments d ON e.department=d.code;
...
SELECT d.name FROM departments d JOIN employees e ON d.code=e.department GROUP BY d.name HAVING COUNT(e.name) > 2;

-- 2.15 Very Important - Select the name and last name of employees working for departments with second lowest budget.
SELECT name, lastname FROM employees WHERE department IN (SELECT code FROM departments ORDER BY budget DESC LIMIT 1 OFFSET 1);
